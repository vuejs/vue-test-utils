...
describeWithShallowAndMount('setProps', mountingMethod => {
  ...
  describe('watchers', () => {
    ...
    xdescribe('immediate watchers', () => {
      const ComponentWithImmediateWatcher = {
        template: `
          <div/>`,
        data() {
          return { data1: null }
      ...
      it('retains the new props after the parent re-renders', async () => {
        const wrapper = mountingMethod(ComponentWithImmediateWatcher, {
          propsData: { props1: 'initial' }
        })
        wrapper.setProps({ props1: 'foo' })
        await wrapper.vm.$nextTick()
        expect(wrapper.vm.$data.data1).to.equal('foo')
      })

      itDoNotRunIf(
        mountingMethod.name === 'shallowMount',
        'works for children components, too',
        async () => {
          const wrapper = mountingMethod(
            {
              template:
                '<div><ComponentWithImmediateWatcher class="child"/></div>',
              components: { ComponentWithImmediateWatcher }
            },
            {
              propsData: { props1: 'initial' }
            }
          ...
          child.setProps({ props1: 'foo' })
          await child.vm.$nextTick()
          expect(child.vm.$data.data1).to.equal('foo')
        }
      )
    })
  })
